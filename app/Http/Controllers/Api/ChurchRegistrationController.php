...
use App\Http\Requests\Api\RegisterChurchRequest;
...
{
    public function store(RegisterChurchRequest $request): JsonResponse
    {
        ...
        $result = DB::transaction(function () use ($validated): array {
            ...
            $church = Church::create([
                ...
                'code' => ($churchData['code'] ?? null) ?: $this->generateChurchCode($churchData['name']),
                ...
                'finance_enabled' => (bool) ($settings['finance_enabled'] ?? false),
                'special_services_enabled' => (bool) ($serviceData['special_services_enabled'] ?? true),
            ]);
            ...
            $this->createServiceSchedules($church, $serviceData);

            return [
                'church' => $church->load('serviceSchedules'),
                'admin' => $admin,
            ];
        });

        return response()->json([
            'message' => 'Church registration completed successfully.',
            'data' => $result,
        ], 201);
    }

    private function createServiceSchedules(Church $church, array $serviceData): void
    {
